...
std::ostream& Radio::print(std::ostream& out) const {
	AudioAndVideo::print(out);
	out << "\tTyner type: " << TunerName[tuner] << std::endl;
	out << "\tMaximum Supported Frequencies: " << SuppFreqName[suppfreq] << std::endl;
	out << "\tPortableness: " << IsPortableName[isportable] << std::endl << std::endl;
	return out;
}
std::ostream& operator<<(std::ostream& out, const Radio& radio) {
	return radio.print(out);
}
